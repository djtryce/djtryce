...
class RightsEngine:
    ...
    def _evaluate_css(self, p: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if household_size != 1 or area != "metropolitan_france":
            return self._result(
                ...
                "Le référentiel actuel du prototype ne contient qu'un barème sécurisé pour une personne seule en métropole.",
            )
        ...
        if annual is None:
            return self._result(
                "css",
                "INFORMATION_INSUFFISANTE",
                "Les ressources annuelles nécessaires à l'évaluation ne sont pas connues.",
                ["income.annual_household_resources"],
            )
        ...
        if annual <= thresholds["free_annual"]:
            return self._result(
                "css",
                "POTENTIEL",
                "Les ressources déclarées sont sous le plafond gratuit enregistré dans le référentiel audité. La caisse reste seule compétente pour attribuer le droit.",
            )
        if annual <= thresholds["contribution_annual"]:
            return self._result(
                "css",
                "POTENTIEL_AVEC_PARTICIPATION",
                "Les ressources déclarées se situent entre les deux plafonds enregistrés dans le référentiel audité.",
            )
        return self._result(
            "css",
            "CONDITIONS_DECLAREES_INCOMPATIBLES",
            "Les ressources déclarées dépassent le plafond actuellement enregistré pour une personne seule en métropole.",
        )
    ...
    def _evaluate_housing(self, p: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if status in {"tenant", "social_residence"}:
            return self._result(
                "apl_alf_als",
                "A_SIMULER",
                "La situation de logement déclarée justifie une simulation officielle. Le montant n'est pas calculé par ce prototype.",
            )
        if status is None or status == "unknown":
            return self._result(
                "apl_alf_als",
                "INFORMATION_INSUFFISANTE",
                "Le statut d'occupation du logement est inconnu.",
                ["housing.status"],
            )
        return None
    ...
    def _evaluate_pch(self, p: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if h.get("health_or_disability_difficulty"):
            return self._result(
                "pch",
                "A_EXPLORER",
                "Des difficultés liées au handicap ou à la santé sont déclarées. L'ouverture de la PCH nécessite une évaluation MDPH/CDAPH.",
            )
        return None
    ...
    def _evaluate_rqth(self, p: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if p["employment"].get("work_limitation_due_to_health_or_disability"):
            return self._result(
                ...
                "Une limitation de travail liée à la santé ou au handicap est déclarée. La décision relève de la MDPH/CDAPH.",
            )
        return None
    ...
    def _evaluate_cmi(self, p: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if p["health_disability"].get("mobility_difficulty"):
            return self._result(
                "cmi",
                "EVALUATION_HUMAINE_NECESSAIRE",
                "Une difficulté de mobilité est déclarée. La mention de CMI applicable ne peut pas être déterminée automatiquement.",
            )
        return None
    ...
    def _evaluate_apa(self, p: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if age < 60:
            return self._result(
                "apa",
                "CONDITIONS_DECLAREES_INCOMPATIBLES",
                "L'âge déclaré est inférieur au seuil d'âge enregistré pour l'APA.",
            )
        if gir is None:
            return self._result(
                "apa",
                "EVALUATION_HUMAINE_NECESSAIRE",
                "L'âge est compatible mais le niveau GIR doit être évalué ou renseigné.",
                ["health_disability.gir_level"],
            )
        if gir <= 4:
            return self._result(
                "apa",
                "EVALUATION_HUMAINE_NECESSAIRE",
                "L'âge et le GIR déclarés sont compatibles avec les critères généraux enregistrés, mais l'attribution reste une décision administrative.",
            )
        return self._result(
            "apa",
            "CONDITIONS_DECLAREES_INCOMPATIBLES",
            "Le GIR déclaré est hors des niveaux 1 à 4 enregistrés pour l'ouverture de l'APA.",
        )
    ...
    def _evaluate_invalidity(self, p: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if assessed is not True:
            return self._result(
                "pension_invalidite",
                "EVALUATION_HUMAINE_NECESSAIRE",
                "Une difficulté de santé est déclarée, mais la réduction de capacité doit être appréciée par le médecin-conseil.",
                ["health_disability.medical_capacity_loss_two_thirds_assessed"],
            )
        return self._result(
            "pension_invalidite",
            "A_EXPLORER",
            "Une réduction de capacité des deux tiers est déclarée comme déjà évaluée. Les autres conditions administratives doivent encore être vérifiées.",
        )
    ...
    def _evaluate_legal_aid(self, p: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if p["household"].get("household_size") != 1:
            return self._result(
                ...
                "A_SIMULER",
                "Une procédure est déclarée, mais le prototype ne contient pas encore les plafonds sécurisés de toutes les compositions de foyer.",
            )
        ...
        if rfr is None:
            return self._result(
                "aide_juridictionnelle",
                "INFORMATION_INSUFFISANTE",
                "Le revenu fiscal de référence est manquant.",
                ["income.reference_tax_income"],
            )
        return self._result(
            "aide_juridictionnelle",
            "A_SIMULER",
            "Une procédure est déclarée. Le revenu et le patrimoine doivent être examinés ensemble avant toute conclusion.",
        )
    ...
    def _evaluate_ajpa(self, p: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if p["family"].get("caregiver_for_relative"):
            return self._result(
                "ajpa",
                "A_EXPLORER",
                "La personne déclare aider un proche. Les conditions d'ouverture doivent être vérifiées avant toute conclusion.",
            )
        return None
